...
Page({
  ...
  initCatelog: function (id) {
    var that = this;
    let url = app.globalData.detailUrl + `?id=${id}`;
    wx.showLoading({
      title: '加载中',
      mark: true
    });
  ...
  toReader:function(e){
    ...
    wx.navigateTo({
      url: `../reader/reader?id=${e.currentTarget.id}&serialNumber=${serialNumber}&title=${that.data.item.Title}`
    });
  },
  ...
  insertBookShelf: function () {
    ...
    console.log(item);
    var info = new BookShelfInfo(that.data.bid, item.bname,
      item.bicon, 1, "","",item.bauthor,item.num);
